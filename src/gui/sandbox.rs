...
impl Sandbox {
    ...
    pub fn render(&mut self, ctx: &egui::Context) {
        if let Some(pkg) = self.pkg.as_mut() {
            let mut is_open = true;
            egui::Window::new("XML Sandbox")
                .resizable(true)
                ...
                .show(ctx, |ui| {
                        ...
                        .show_inside(ui, |ui| {
                            ui.with_layout(ui.layout().clone().with_cross_justify(true), |ui| {
                                ...
                                egui::ScrollArea::vertical().show_rows(
                                    ...
                                    |ui, range| {
                                        for (i, name) in
                                            self.filtered_pkg_names.iter().skip(range.start).take(range.len())
                                        {
                    ...
                    if let Some(output) = self.output.as_ref() {
                        egui::SidePanel::right("sandbox output")
                            .min_width(300.0)
                            .show_inside(ui, |ui| match output {
                    ...
                    egui::TopBottomPanel::bottom("egui bug workaround #1")
